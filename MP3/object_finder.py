...
def finderSIFT(sample, video):
    ...
    result = cv2.VideoWriter('WMA/MP3/resultSIFT.avi',  cv2.VideoWriter_fourcc(*'MJPG'), 20, size)
...
def finderORB(sample, video):
    ...
    frame_height = int(video.get(4))
    size = (frame_width, frame_height)
    result = cv2.VideoWriter('WMA/MP3/resultORB.avi',  cv2.VideoWriter_fourcc(*'MJPG'), 20, size)
...
def main():
    sample = cv2.imread('WMA/MP3/resourses/sample.jpg')
    sample = resize(sample, 4)   
    cap = cv2.VideoCapture('WMA/MP3/resourses/video_sourse.mp4')
